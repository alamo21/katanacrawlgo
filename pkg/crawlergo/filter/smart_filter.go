...
import (
	...
	model2 "katanacrawlgo/pkg/crawlergo/model"
	"katanacrawlgo/pkg/crawlergo/tools"
	"log"
	"regexp"
	"sort"
	"strings"
...
需要过滤则返回 true
*/
func (s *SmartFilter) DoFilter(req *model2.Request) bool {
	...
	if req.Method == config.GET || req.Method == config.DELETE || req.Method == config.HEAD || req.Method == config.OPTIONS {
		...
		log.Println(chalk.Red.Color("error: 不支持该请求方式: " + req.Method))
	}
...
对GET请求的参数和路径进行标记
*/
func (s *SmartFilter) getMark(req *model2.Request) {
	// 首先是解码前的预先替换
	todoURL := *(req.URL)
...
对POST请求的参数和路径进行标记
*/
func (s *SmartFilter) postMark(req *model2.Request) {
...
标记参数值
*/
func (s *SmartFilter) markParamValue(paramMap map[string]interface{}, req model2.Request) map[string]interface{} {
	markedParamMap := map[string]interface{}{}
	for key, value := range paramMap {
		switch value.(type) {
		case bool:
...
全局数值型参数过滤
*/
func (s *SmartFilter) globalFilterLocationMark(req *model2.Request) {
...
之后对超过阈值的部分再次打标记
*/
func (s *SmartFilter) repeatCountStatistic(req *model2.Request) {
...
*/
func (s *SmartFilter) overCountMark(req *model2.Request) {
	queryKeyId := req.Filter.QueryKeysId
	pathId := req.Filter.PathId
	// 参数不为空，
...
func (s *SmartFilter) calcFragmentID(fragment string) string {
	...
	fakeUrl, err := model2.GetUrl(fragment)
	if err != nil {
		log.Println(chalk.Red.Color("error: " + err.Error()))
		return ""
	}
	// XXX: discuss https://katanacrawlgo/issues/100
	fakeReq := model2.GetRequest(config.GET, fakeUrl)
	s.getMark(&fakeReq)
...
计算标记后的唯一请求ID
*/
func getMarkedUniqueID(req *model2.Request) string {
	var paramId string
	if req.Method == config.GET || req.Method == config.DELETE || req.Method == config.HEAD || req.Method == config.OPTIONS {
