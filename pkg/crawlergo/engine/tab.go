...
func (tab *Tab) Start() {
	log.Println(chalk.Green.Color("Crawling crawlergo " + tab.NavigateReq.Method + ": " + tab.NavigateReq.URL.String()))
	defer tab.Cancel()
...
func (tab *Tab) Evaluate(expression string) {
	...
	defer cancel()
	runtime.Evaluate(expression).Do(tCtx)
}
